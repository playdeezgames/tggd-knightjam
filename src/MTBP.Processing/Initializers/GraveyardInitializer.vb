...
Friend Module GraveyardInitializer
    ...
    Friend Sub Initialize(world As IWorld, context As IInitializationContext)
        ...
        graveYardEntrance.Inventory.CreateItem(AddressOf InitializePotion)
    ...
    Private Sub InitializePotion(item As IItem)
        item.SetName("Heath Potion")
        item.SetDescription("This is vial filled with a vile looking liquid. It kinda looks like chocolate. I don't think that's chocolate.")
        item.CreateVerb(AddressOf QuaffPotion)
    End Sub

    Private Sub QuaffPotion(verb As IVerb)
        verb.SetName("Quaff")
        verb.SetDescription("Yeah, that's right. Quaff, not drink. Learn yer roguelike history.")
        verb.SetVerbType(VerbTypes.QUAFF_POTION)
    End Sub
